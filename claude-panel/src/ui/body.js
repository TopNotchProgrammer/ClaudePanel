...
<header>
  ...
  <div id="usage-strip" class="usage-strip" style="display:none">
    <div class="usage-row" id="usage-session">
      <span class="usage-label">${t.usageSession}</span>
    ...
    <div class="usage-row" id="usage-week">
      <span class="usage-label">${t.usageWeek}</span>
      <div class="usage-bar"><div class="usage-fill"></div></div>
      <span class="usage-pct">—</span>
      <span class="usage-reset"></span>
    </div>
  ...
  <div id="search-wrap" class="row" style="display:none;margin-top:8px">
    <input id="q" type="search" placeholder="${t.searchPlaceholder}" autocomplete="off">
  </div>
  <div id="filter-wrap" class="tabs" style="display:none">
    <select id="filter-select" class="filter-select">
      <option value="all">${t.filterAll}</option>
      <option value="user">${t.filterUser}</option>
      <option value="assistant">${t.filterAssistant}</option>
      <option value="tool_use">${t.filterTools}</option>
      <option value="tool_result">${t.filterResults}</option>
      <option value="thinking">${t.filterThinking}</option>
    </select>
  </div>
...
<form id="send-form" class="send-form" style="display:none" autocomplete="off">
  <div class="send-inner">
    ...
    <div class="send-row">
      <button id="attach-btn" type="button" class="icon-btn" aria-label="${t.attachFile}" title="${t.attachFile}">
      ...
      <button id="interrupt-btn" type="button" class="icon-btn interrupt-btn" aria-label="${t.interruptClaude}" title="${t.interruptClaude}" disabled>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
      </button>
      <textarea id="send-text" rows="1"></textarea>
      <button id="send-btn" class="submit" type="submit">${t.sendBtn}</button>
    </div>
  </div>
</form>
`;
